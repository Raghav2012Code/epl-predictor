    ...
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          const baseline = scale(0) ?? 0;
          barW = Math.abs(valuePos - baseline);
          barHeight = barWidth;

          if (stacked && stackOffsets) {
            const offset = stackOffsets.get(i)?.get(dataKey) ?? 0;
            const startX = scale(offset) ?? 0;
            const endX = scale(offset + value) ?? startX;
            x = Math.min(startX, endX);
            barW = Math.abs(endX - startX);
          ...
          const valuePos = scale(value) ?? 0;
          const baseline = scale(0) ?? innerHeight;
          barHeight = Math.abs(valuePos - baseline);
          ...
          if (stacked && stackOffsets) {
            ...
            const endY = scale(offset + value) ?? offsetY;
            // Apply stack gap: shift up and reduce height
            const gapOffset = seriesIndex * stackGap;
            y = Math.min(endY, offsetY) + gapOffset;
            barHeight = Math.abs(offsetY - endY);
