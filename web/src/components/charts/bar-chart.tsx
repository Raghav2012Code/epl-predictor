...
const ChartCore = memo(function ChartCore({
  ...
  const categoryScale = useMemo(() => {
    ...
    const range: [number, number] = isHorizontal
      ? [0, innerHeight]
      : [0, innerWidth];
    const slot = (isHorizontal ? innerHeight : innerWidth) / Math.max(1, data.length);
    const padding = barWidthProp == null
      ? barGap
      : Math.min(barGap, Math.max(0, 1 - barWidthProp / Math.max(1, slot)));
    return scaleBand<string>({
      range,
      domain,
      padding,
    });
  }, [innerWidth, innerHeight, data, categoryAccessor, barGap, barWidthProp, isHorizontal]);

  // Band width for bars - a supplied width is a real rendered width, while
  // the band scale still controls each category's position.
  ...
  const valueScale = useMemo(() => {
    ...
    return scaleLinear({
      range,
      domain: [
        data.some((d) => lines.some((line) => typeof d[line.dataKey] === "number" && Number(d[line.dataKey]) < 0))
          ? -maxValue * 1.1
          : 0,
        maxValue * 1.1,
      ],
  ...
  }, [data, innerWidth, innerHeight, lines, maxValue, isHorizontal]);
  ...
  const yScales = useMemo(() => {
    ...
    return buildYScalesForLines({
      ...
      resolveDomain: (dataKeys) => {
        if (stacked) {
          ...
          for (const d of data) {
            ...
            for (const key of dataKeys) {
              const value = typeof d[key] === "number" ? Number(d[key]) : 0;
              if (value < 0) negative += value;
              else positive += value;
            }
            positiveMax = Math.max(positiveMax, positive);
            negativeMin = Math.min(negativeMin, negative);
          }
          return [negativeMin * 1.1 || 0, positiveMax * 1.1 || 100];
        }
        let max = 0;
        for (const d of data) {
          for (const key of dataKeys) {
        ...
        const extent = (max || 100) * 1.1;
        return [
          data.some((d) => dataKeys.some((key) => Number(d[key]) < 0))
            ? -extent
            : 0,
          extent,
        ];
  ...
  const stackOffsets = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (const line of lines) {
        const value = d[line.dataKey];
        if (typeof value === "number") {
          const cumulative = value < 0 ? negativeCumulative : positiveCumulative;
  ...
  const columnWidth = useMemo(
    () => (data.length > 0 ? (isHorizontal ? innerHeight : innerWidth) / data.length : 0),
    [data.length, innerHeight, innerWidth, isHorizontal],
  );
  ...
  const fakeTimeScale = useMemo(() => {
    ...
    const scale = Object.assign(
      (value: Date | number) => {
        ...
        return ((timestamp - start) / Math.max(1, now - start)) * innerWidth;
      },
      {
  ...
  }, [animationDuration, animationEasing, enterTransition, revealSignature, status]);
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent<SVGGElement>) => {
      ...
      // Resolve against actual scale-band centers, including padding.
      let clampedIndex = 0;
      let nearestDistance = Number.POSITIVE_INFINITY;
      data.forEach((candidate, index) => {
        const start = categoryScale(categoryAccessor(candidate)) ?? 0;
        const distance = Math.abs(pos - (start + bandWidth / 2));
      ...
      if (isHorizontal) {
        ...
        const maxX = Object.values(xPositions).length > 0
          ? Math.max(...Object.values(xPositions))
          : 0;
        tooltipX = maxX;
      } else {
