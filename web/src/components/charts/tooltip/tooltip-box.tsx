...
function TooltipBoxInner({
  ...
  const finalLeft = animate
    ...
    : (typeof leftOverride === "number" ? leftOverride : staticPosition.left);
  const finalTop = animate
    ? (topOverride ?? animatedTop)
    : (typeof topOverride === "number" ? topOverride : staticPosition.top);
  ...
  if (!entrance) {
    return createPortal(
      <div
        ...
        ref={tooltipRef}
        style={{ left: finalLeft as number, top: finalTop as number }}
      >
